...
|Тип|Описание|
|--|--|
|Пробная подписка|Создайте _бесплатную_ [пробную подписку](https://azure.microsoft.com/free/).|
|Существующая подписка|Если у вас уже есть подписка, получите доступ к существующей подписке с помощью [портала Azure](https://portal.azure.com), [Azure CLI]() или JavaScript.|
|Несколько подписок|Если вам нужно управлять несколькими подписками, [узнайте](/azure/governance/management-groups/create-management-group-javascript), как создать группу управления с помощью JavaScript.|

## <a name="one-time-software-installation"></a>Одноразовая установка программного обеспечения
...
### <a name="recommended-local-installations"></a>Рекомендуемые локальные установки

Следующие необязательные общие установки рабочей станции рекомендуется использовать для выполнения задач локальной разработки.

|Имя или установщик|Описание|
|--|--|
|[Azure CLI](/cli/azure/get-started-with-azure-cli) или [расширения Visual Studio Code для Azure](https://marketplace.visualstudio.com/search?term=azure&target=VSCode&category=All%20categories&sortBy=Relevance) |Работа с Azure обычно осуществляется через [портал Azure](https://ms.portal.azure.com/), Azure CLI или определенные расширения Visual Studio Code для Azure. Хотя вам необязательно устанавливать Azure CLI (если это не требуется для работы с руководством или учебником), этот инструмент включает все функции для работы с Azure, а Visual Studio Code предоставляет аналогичную функциональность в виде расширений для конкретных служб.|
...
## <a name="working-with-azure-and-the-azure-sdk-client-libraries"></a>Работа с Azure и клиентскими библиотеками Azure SDK
...
- Создайте ресурсы Azure и сохраните связанные ключи или конфигурацию в [безопасном расположении](#securing-configuration-information).
...
- [Dotenv](https://www.npmjs.com/package/dotenv) — популярный пакет npm для считывания переменных среды из файла `.env`. Обязательно добавьте файл `.env` к файлу `.gitignore`, чтобы файл `.env` не синхронизировался с системой управления версиями. Узнайте больше о [переменных среды](../how-to/configure-web-app-settings.md) в веб-приложениях для Azure. 
- Azure [Key Vault](/azure/key-vault/) используется для создания и обслуживания ключей, используемых для получения доступа к облачным ресурсам, приложениям и решениям, а также их шифрования.

### <a name="create-environment-variables-for-the-azure-libraries"></a>Создание переменных среды для библиотек Azure

Чтобы использовать параметры Azure, которые требуются библиотекам Azure SDK для доступа к облаку Azure, задайте для [переменных среды](../how-to/configure-web-app-settings.md) обычные значения. Следующие команды задают переменные среды на локальной рабочей станции. Еще один распространенный механизм — использовать пакет NPM `DOTENV` для создания файла `.env` для этих параметров. Если вы планируете использовать файл `.env`, убедитесь, что он не синхронизируется с системой управления версиями. Стандартным способом обеспечения синхронизации этих параметров с системой управления версиями является добавление файла `.env` в файл Git `.ignore`.
...
* [Развертывание статического веб-сайта в Azure из Visual Studio Code](../tutorial/tutorial-vscode-static-website-node/tutorial-vscode-static-website-node-01.md)