...
* [Исходный код](https://github.com/Azure-Samples/js-e2e-client-azure-login-button)
...
Зарезервируйте место для копирования значения идентификатора клиента приложения. 
...
1. Перейдите в новый каталог и установите пакет `@azure/msal-browser`:

   ```bash
   cd tutorial-demo-login-button && npm install @azure/msal-browser
   ```

1. Создайте `.env` в файле корневого уровня и добавьте следующие строки:

    :::code language="env" source="~/../js-e2e-client-azure-login-button/.env"  :::

    Файл `.env` считывается как часть платформы create-react-app.
...
* Продолжайте использовать [библиотеки MSAL](https://docs.microsoft.com/azure/active-directory/develop/msal-overview) для получения профиля пользователя и обеспечения автоматического входа
* Добавьте [Microsoft Graph](https://docs.microsoft.com/graph/overview), чтобы получить доступ к учетным записям пользователей в Microsoft 365, включая электронную почту и встречи в календаре