...
Вместо локального интерфейса командной строки или PowerShell можно использовать Azure Cloud Shell непосредственно через [https://shell.azure.com/](https://shell.azure.com/). Работать с Cloud Shell удобно, так как эта служба автоматически аутентифицируется в Azure после запуска и имеет те же возможности, которые доступны на портале Azure. Но так как Cloud Shell не является локальной средой, она лучше подходит для отдельных операций, которые вы выполняете на портале, а не для автоматизации с использованием скриптов.
