...
description: Сведения о настройке локальной среды разработки Python для работы с Azure, включая Visual Studio Code, пакет Azure SDK и необходимые учетные данные для проверки подлинности пакета SDK.
ms.date: 05/12/2020
ms.topic: conceptual
...
- [Настройте проверку подлинности](#configure-authentication) для использования библиотеки Azure SDK, чтобы подготовить к работе ресурсы Azure, управлять ими и получить к ним доступ.
...
Azure CLI обычно поддерживает вход в разные сеансы, но рекомендуется запускать `az login` при каждом открытии нового терминала или командной строки.

## <a name="configure-authentication"></a>Настройка аутентификации

Как описано в разделе [Основные сведения об авторизации Azure ](how-to-manage-service-principals.md#basics-of-azure-authorization), каждому разработчику требуется субъект-служба для использования в качестве удостоверения приложения при локальном тестировании кода приложения.

В следующих разделах описывается создание субъекта-службы и переменных среды, которые предоставляют свойства субъекта-службы для пакета Azure SDK.

Каждый разработчик в организации должен выполнить эти шаги индивидуально.

### <a name="create-a-service-principal-for-development"></a>Создание субъекта-службы для разработки
...
1. Создайте субъект-службу, выполнив следующую команду:
    ...
    - `ad` означает Azure Active Directory; `sp` означает "субъект-служба", а `create-for-rbac` означает "создать для управления доступом на основе ролей" (основная форма авторизации Azure). См. справочник по командам [az ad sp create-for-rbac](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac).
    ...
    - Команда предоставляет выходные данные JSON, которые сохраняются в файле *local-sp.json*.
...
Чтобы позже изменить или удалить субъекты-службы, ознакомьтесь со статьей [Управление субъектами-службами](how-to-manage-service-principals.md).

### <a name="create-environment-variables-for-the-azure-sdk"></a>Создание переменных среды для пакета Azure SDK
...
Чтобы получить идентификатор подписки, выполните команду [`az account show`](/cli/azure/account?view=azure-cli-latest#az-account-show) и найдите свойство `id` в выходных данных.

Для удобства создайте файл *SH* или *CMD* с помощью этих команд, которые можно запускать при каждом открытии терминала или командной строки для локального тестирования. Опять же, не добавляйте файл в систему управления версиями, чтобы он оставался только в пределах вашей учетной записи пользователя.
...
Теперь, когда ваша локальная среда разработки готова, давайте кратко рассмотрим пакет SDK для Azure.

> [!div class="nextstepaction"]
> [Использование пакета Azure SDK для Python >>>](azure-sdk-overview.md)
