...
1. На странице **Создание** выберите **Веб-приложение** в столбце **Популярное**. (Если пункт **Веб-приложение** не отображается, выберите **Интернет** в разделе **Azure Marketplace**, а затем **Веб-приложение** в разделе **Подборка приложений**.) 
...
1. На странице **Создание** выберите **База данных** в разделе **Azure Marketplace**, а затем **База данных Azure для PostgreSQL** в разделе **Подборка приложений**.
...
1. На странице **Отдельный сервер** введите следующие сведения:
    ...
    | Имя сервера | Имя сервера базы данных, уникальное во всех службах Azure (URL-адрес приложения: `https://\<server-name>.postgres.database.azure.com`). Допустимые символы: `A`-`Z`, `0`-`9` и `-`. Рекомендуется использовать сочетание названия компании и идентификатора сервера. |
    ...
    | Вычисления и хранение | Выберите **Настроить сервер**, **Базовый** и **5-е поколение**. Задайте для параметра **Виртуальное ядро** значение 1, для параметра **Хранилище** — значение 5 ГБ, а затем нажмите **OK**. Эта конфигурация позволяет подготовить самый недорогой сервер для PostgreSQL в Azure. |
    | Имя администратора, пароль, подтверждение пароля | Введите учетные данные для учетной записи администратора на сервере базы данных. Запишите эти учетные данные, так как они понадобятся вам позже при работе с этим руководством. |
...
1. В Cloud Shell введите следующую команду.
    ...
    Замените `<server-name>` и `<user-name>` именами, которые вы использовали ранее при настройке сервера. Обратите внимание, что полное имя пользователя имеет следующий формат: `<user-name>@<server-name>`.
...
1. С помощью кнопки **Создать параметр приложения** создайте параметры для каждого из следующих значений:
    ...
    | DBHOST | URL-адрес сервера базы данных из предыдущего раздела в виде `<server-name>.postgres.database.azure.com`. Вы можете скопировать URL-адрес целиком на странице обзора для сервера базы данных. |
    | DBNAME | `pollsdb` |
    | DBUSER | Полное имя администратора, которое вы использовали ранее. Полное имя пользователя: `<user-name>@<server-name>`. |
...
1. На шаге **Управление исходным кодом** выберите **GitHub**, затем выполните вход или выберите **Продолжить**, чтобы использовать текущее имя для входа в GitHub.
...
1. В окне браузера или на вкладке веб-приложения выберите **SSH** в разделе **Средства разработки** слева, чтобы открыть консоль SSH на сервере веб-приложения. Первое подключение может занять некоторое время, так как предварительно нужно запустить контейнер веб-приложения.
