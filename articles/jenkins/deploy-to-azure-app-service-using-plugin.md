...
При установке Jenkins убедитесь, что установлены следующие подключаемые модули:
...
1. Чтобы добавить субъект-службу Microsoft Azure, выберите **Add Credentials** (Добавить учетные данные). Укажите значения в полях **идентификатора подписки** , **идентификатора клиента** , **секрета клиента** и **конечной точки маркера OAuth 2.0** . Задайте **mySp** в качестве значения поля **идентификатора** . Мы будем использовать этот идентификатор в последующих шагах этой статьи.
...
1. Создайте веб-приложение. Воспользуйтесь [порталом Azure](/azure/app-service/configure-common) или выполните следующую команду `az` Azure CLI:

    ```azurecli
    az webapp create --name <myAppName> --resource-group <myResourceGroup> --plan <myAppServicePlan>
    ```
    
1. Настройте конфигурацию среды выполнения Java, необходимую вашему приложению. Следующая команда Azure CLI настраивает веб-приложение для запуска в JDK 8 и [Apache Tomcat](https://tomcat.apache.org/) версии 8.0.
...
1. Настройте поле **управления исходным кодом** для использования локальной вилки [простого веб-приложения Java для Azure](https://github.com/azure-devops/javawebappsample). Укажите значение **URL-адреса репозитория** . Например, http:\//github.com/&lt;your_ID>/javawebappsample.

1. Добавьте шаг для создания проекта с помощью Maven. Для этого добавьте команду **оболочки выполнения** . В этом примере требуется дополнительная команда для переименования \*WAR-файла в целевой папке на **ROOT.war** .   
...
1. Добавьте действие после сборки, выбрав **Publish an Azure Web App** (Публикация веб-приложения Azure).

1. Укажите **mySp** в качестве субъекта-службы Azure. Этот субъект был сохранен в качестве [учетных данных Azure](#service-principal) на предыдущем шаге.

1. В разделе **конфигурации приложения** выберите группу ресурсов и веб-приложение в своей подписке. Подключаемый модуль Jenkins автоматически определяет платформу веб-приложения (Windows или Linux). Для веб-приложения Windows появляется параметр **Publish Files** (Публикация файлов).

1. Заполните файлы, которые нужно развернуть. Например, укажите пакет в формате WAR, если вы используете Java. Используйте дополнительные параметры **исходного** и **целевого каталогов** , чтобы указать исходные и целевые папки для отправки файла. Веб-приложение Java в Azure выполняется на сервере Tomcat. Поэтому для Java необходимо отправить пакет в формате WAR в папку веб-приложений. В этом примере задайте для **исходного каталога** значение **target** , а для **целевого каталога** — **webapps** .

1. Если вы хотите выполнить развертывание в слот, отличный от рабочего, можно также задать имя **слота** .

1. Сохраните проект и создайте его. Веб-приложение развертывается в Azure после сборки.
...
1. Задайте определение **withCredentials** в строке 14 для идентификатора учетных данных в вашем экземпляре Jenkins.
...
1. Укажите имя задания и выберите **Pipeline** (Конвейер). Щелкните **ОК** .

1. Откройте вкладку **Pipeline** (Конвейер).

1. Для значения **Definition** (Определение) выберите значение **Pipeline script from SCM** (Сценарий конвейера из SCM).

1. Для значения **SCM** выберите **Git** . Введите URL-адрес GitHub для разветвленного репозитория. Например: https://&lt;ваш_разветвленный_репозиторий>.git.

1. Измените значение **пути к сценарию** на **Jenkinsfile_ftp_plugin** .

1. Нажмите кнопку **Сохранить** (Сохранить) и запустите задание.
...
1. Добавьте шаг для создания проекта с помощью Maven. Для этого добавьте команду **оболочки выполнения** . Добавьте следующую строку в команду:
...
1. В разделе **конфигурации приложения** выберите группу ресурсов и веб-приложение Linux в своей подписке.

1. Выберите **Publish via Docker** (Публикация через Docker).

1. Заполните значение пути **Dockerfile** . Вы можете сохранить значение по умолчанию /Dockerfile. При использовании реестра контейнеров Azure для значения **URL-адреса реестра Docker** укажите URL-адрес в формате https://&lt;ваш_реестр>.azurecr.io. Если вы используете DockerHub, оставьте это значение пустым.

1. Для значения **учетных данных реестра** добавьте учетные данные для реестра контейнеров. Идентификатор пользователя и пароль можно получить, выполнив следующие команды в Azure CLI. Первая команда включает учетную запись администратора.
...
1. Имя образа Docker и значение тега на вкладке **Дополнительно** являются необязательными. По умолчанию значение имени образа можно получить из имени образа, настроенного на портале Azure (в параметрах **контейнера Docker** ). Тег создается на основе $BUILD_NUMBER. Укажите имя образа на портале Azure или задайте значение **образа Docker** на вкладке **Дополнительно** . В этом примере задайте &lt;ваш_реестр>.azurecr.io/calculator в качестве значения **образа Docker** и не указывайте значение **тега образа Docker** .

1. Развертывание завершается сбоем, если использовать параметр встроенного образа Docker. Настройте конфигурацию Docker для использования пользовательского образа в настройках **контейнера Docker** на портале Azure. Для развертывания встроенного изображения используйте подход с передачей файла.

1. Кроме того, можно выбрать имя **слота** , отличное от **рабочего** . Этот способ аналогичный способу передачи файла.

1. Сохраните и создайте проект. После этого образ контейнера помещается в реестр, а веб-приложение развертывается.
...
1. Измените строку 13 на сервер контейнера реестра:

    ```java
    def registryServer = '<registryURL>'
    ```

1. Измените строку 16 для использования идентификатора учетных данных в вашем экземпляре Jenkins:
...
1. Для значения **SCM** выберите **Git** . Введите URL-адрес GitHub для разветвленного репозитория. Например: `https://&lt;your_forked_repo>.git.`

1. Измените значение **пути к сценарию** на **Jenkinsfile_container_plugin** .

1. Нажмите кнопку **Сохранить** (Сохранить) и запустите задание.

## <a name="verify-your-web-app"></a>Проверка веб-приложения

1. Чтобы проверить успешное развертывание WAR-файла в веб-приложении, откройте веб-браузер. Перейдите по адресу `http://&lt;your_app_name>.azurewebsites.net/api/calculator/ping`. Замените `&lt;your_app_name>` именем веб-приложения. Отобразится следующее сообщение:
...
1. Перейдите по адресу `http://&lt;your_app_name>.azurewebsites.net/api/calculator/add?x=&lt;x>&y=&lt;y>`. Замените `&lt;x> and &lt;y>` любыми числами для получения сумы x + y. Калькулятор отображает сумму:
...
1. Перейдите по адресу `http://&lt;your_app_name>.azurewebsites.net/api/calculator/ping`. Замените `&lt;your_app_name>` именем веб-приложения. Отобразится следующее сообщение:
...
1. Перейдите по адресу `http://&lt;your_app_name>.azurewebsites.net/api/calculator/add?x=&lt;x>&y=&lt;y>`. Замените `&lt;x> and &lt;y>` любыми числами для получения сумы x + y.
