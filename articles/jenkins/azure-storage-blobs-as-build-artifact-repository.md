...
В рамках этого руководства вы примените подключаемый модуль службы хранилища Azure для решения непрерывной интеграции Jenkins, предоставляемый корпорацией Майкрософт.

## <a name="jenkins-overview"></a>Общие сведения о Jenkins

Jenkins обеспечивает непрерывную интеграцию программного проекта, позволяя разработчикам автоматизировать интеграцию изменений кода и построения сборок. Этот шаблон упрощает работу и повышает производительность. Для сборок применяется контроль версий, а артефакты сборки можно загружать в разные репозитории. В этой статье показано, как применять Хранилище BLOB-объектов Azure в качестве репозитория артефактов сборки. Кроме того, вы узнаете, как скачать зависимости из Хранилища BLOB-объектов Azure.
...
* высокая доступность артефактов сборки или загружаемых зависимостей;
...
* контроль над политиками доступа пользователей с возможностью выбрать такие варианты, как анонимный доступ, доступ по подписанному URL-адресу с ограниченным сроком действия и закрытый доступ.
...
1. Если у вас нет решения Jenkins, его можно запустить следующим образом:
  ...
  3. Откройте панель мониторинга Jenkins по адресу `http://localhost:8080/`. Панель мониторинга Jenkins нужна для установки и настройки подключаемого модуля службы хранилища Azure.
...
8. Щелкните **Make new container public by default** (Сделать новый контейнер общедоступным по умолчанию) для этого примера. (Если вы хотите использовать частный контейнер, необходимо создать подписанный URL-адрес. Это выходит за рамки данной статьи. Дополнительные сведения о подписанных URL-адресах см. в статье [Использование подписанных URL-адресов (SAS)](/azure/storage/common/storage-sas-overview)).
9. [Необязательно.] Щелкните элемент **Clean container before uploading** (Очистить контейнер перед передачей), если хотите, чтобы контейнер был очищен перед передачей артефактов сборки (не устанавливайте этот флажок, если не хотите очищать содержимое контейнера).
...
- Указанный выше формат применяется для глобального облака Azure. При использовании другого облака Azure используйте конечную точку на [портале Azure](https://portal.azure.com), чтобы определить конечную точку URL-адреса.
- Замените заполнитель `storageaccount` именем учетной записи хранения.
- Замените заполнитель `container_name` именем контейнера.
- Замените заполнитель `blob_name` именем большого двоичного объекта.
- В имени контейнера может быть несколько путей. Эти пути разделяются косой чертой. 
- Например, для работы с этим руководством используется контейнер с именем **MyJob**.
- Обычно виртуальный путь задается в формате **${ИД\_СБОРКИ}/${НОМЕР\_СБОРКИ}** . Такое значение позволяет создать для большого двоичного объекта URL-адрес следующего вида:
